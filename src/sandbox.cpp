...
void enter_sandbox(const Flags &flags) {
  ...
  { // basic rules applies to all commands
    seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(write), 1,
                     SCMP_A0(SCMP_CMP_EQ, STDOUT_FILENO));
    seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(write), 1,
                     SCMP_A0(SCMP_CMP_EQ, STDERR_FILENO));
    seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(newfstatat), 1,
                     SCMP_A0(SCMP_CMP_EQ, STDOUT_FILENO));
  ...
  if (flags.advanced) {
    ...
    seccomp_rule_add(ctx, SCMP_ACT_ALLOW, SCMP_SYS(lstat),
                     0); // TODO: remove after removing chroot jail?
